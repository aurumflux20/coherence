...
def report_markdown(c: Coherence) -> str:
    ...
    lines.append("")
    lines.append(
        "[![coherence proof gate]"
        f"({r['shields_url']})](https://github.com/marketplace/actions/coherence-proof-gated-prs)"
    )
    lines.append("")
    lines.append("<details><summary>Add this gate to your repo (3 lines)</summary>")
    lines.append("")
    lines.append("```yaml")
    lines.append("- uses: aurumflux20/coherence@v1")
    lines.append("  with:")
    lines.append("    prove: |")
    lines.append("      pytest -q   # your real check here")
    lines.append("```")
    lines.append("")
    lines.append("Agents can't fake green: sessions are hash-chained; forged results exit 3.")
    lines.append("</details>")
    lines.append("")
    lines.append("_Generated by [Coherence](https://github.com/aurumflux20/coherence) — evidence, not claims._")
    return "\n".join(lines)
